...
export const ServingsTable: React.FC<ServingsTableProps> = ({
  ...
  return (
    <table className="ServingsTable" onScroll={scrolled}>
      <thead>
        <tr>
          <th className="date">{i18n[language].servingsTable.date}</th>
          <th className="servingType">
            {i18n[language].servingsTable.servingName}
          </th>
          <th className="amount">{i18n[language].servingsTable.amount}</th>
          <th className="units">{i18n[language].servingsTable.units}</th>
        </tr>
      </thead>
      <tbody>
        {servings ? (
          servings.map((serving: any) => {
            return (
              <ServingsTableRow
                ...
                updateEditServingFunction={updateEditServingFunction}
