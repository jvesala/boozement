      ...
      >
        ...
        <div>
          <input
            ...
            required
            onChange={(e) =>
              handleFieldUpdate(e, dispatch, updateWeight, setWeightValid)
            }
          />
          <em className="weightTitle">{i18n[language].userdata.weightTitle}</em>
          <Error
            visible={!weightValid}
        ...
        {userdataResult ? (
          <div className="result">{i18n[language].userdata.result}</div>
        ) : (
          ''
        )}

        <Error visible={userdataError} text={i18n[language].userdata.error} />
      </form>
    </div>
  );
};
